...
namespace MessagesApp.Consumer.Consumers
{
    public class MessageConsumer : BackgroundService
    {
        ...
        public MessageConsumer(IServiceProvider? serviceProvider, AuthService? authService, MessagesService? messageService,
            LogMensagensPersistence? logMensagensPersistence, IOptions<RabbitMQSettings>? rabbitMQSettings)
        {
                ...
                using (var scope = _serviceProvider?.CreateScope())
                {
                    switch (messageModel.Tipo)
                    {
                        case 1:
                            ...
                            var messageResponse = await _messageService?.Post(model, authResponse.Token);
